...
server = 'http://192.168.108.19:5000'
srv = ServerProxy(server + '/xmlrpc/', allow_none=True)
...
def paste_id_from_url(url):
    regex = re.compile(r'^%s//show/([^/]+)/?$' % server)
    m = regex.match(url)
    if m is not None:
        return m.group(1)
...
# new paste or reply
    ...
    url = '%s/show/%s' % (server, paste_id)

    print 'Pasted #%s to %s' % (paste_id, url)
    vim.command(":call setreg('p', %r)" % url)
